6/13/2026 - First day of work on this project
- Created name "InsightFlow", may get changed but I like this for now.
- Created an excel spreadsheet with dummy electrician company data to use for basic testing.
- Ability to read excel spreadsheet so that we can use the data inside.
- Able to upload file to streamlit UI.
- Created boxes for lines of sheets and also visual graphs.
- Added a virtual recommendator using Groq model "llama-3.3-70b-versatile" (current working free version).
- Created repo with Github to store version control and can be used to download again if something corrupts.



6/14/2026
